...
const getAssignments = async (req, res) => {
  try {
    ...
    const result = await client.query(
      "SELECT * FROM assignments where batch = $1 and modulecode=$2 ",
      [parseInt(batch), modulecode]
    );
...
const HandleNewAssignment = async (req, res) => {
  try {
    ...
    await client.query(
      " INSERT INTO public.assignments (batch, modulecode, assignmenttitle, assignmentdate, schemeid, schemepath)VALUES ($1, $2,$3,$4,$5,$6) ",
      [
        batch,
        modulecode,
        assignmenttitle,
...
const ChangeScheme = async (req, res) => {
  try {
    ...
    const assignment = (
      await client.query(
        "SELECT * FROM assignments WHERE assignmentid = $1 AND modulecode=$2 AND batch= $3 ",
        [id, modulecode, batch]
      )
    ).rows[0];
    ...
    await client.query(
      "UPDATE assignments SET schemepath=$1 where schemeid=$2",
      [req.file.originalname, assignment.schemeid.toString()]
    );
...
const Update = async (req, res) => {
  try {
    ...
    await client.query(
      "UPDATE assignments assignmenttitle= $1 WHERE modulecode = $2 batch=$3 assignmentid=$4",
      [title, modulecode, batch, id]
    );
