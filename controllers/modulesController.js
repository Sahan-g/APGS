...
const getModules = async (req, res) => {
  const userid = (
    await client.query("SELECT userid FROM users WHERE email = $1", [req.user])
  ).rows[0].userid;
...
const AddModule = async (req, res) => {
  ...
  const result = await client.query(
    "SELECT COUNT(*) FROM modules WHERE modulecode = $1",
    [modulecode]
  );
  ...
  await client.query(
    "INSERT INTO modules (modulecode, modulename, credits) VALUES ($1, $2, $3)",
    [modulecode, modulename, parseInt(credits)]
  );
  ...
  await client.query(
    "INSERT INTO lecturer_modules (userid, modulecode) VALUES ($1, $2)",
    [userid, modulecode]
  );

  res.status(201).json("Module Created Successfully");
};
...
const GetModule = async (req, res) => {
  ...
  const info = (
    await client.query("SELECT * FROM modules WHERE modulecode =$1", [
      modulecode,
    ])
  ).rows[0];
  if (info) {
...
const EditModule = async (req, res) => {
  ...
  const dbmodule = await client.query(
    "SELECT * FROM modules WHERE modulecode = $1",
    [modulecode]
  );
  ...
  await client.query(
    "UPDATE  modules set modulecode=$1, modulename=$2, credits=$3  WHERE modulecode=$4",
    [
